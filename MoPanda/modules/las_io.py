...
class LasIO(LASFile):
    ...
    def check_alias_match(self, new_curves):
        """
        Check if all new_curves have matching keys in the object's self.keys().

        Parameters:
            new_curves (list): A list of new curve names.

        Returns:
            bool: True if all new curves have matching keys, False otherwise.
        """
        ...
        alias_mapping = []  # To store the mapping of new_curve to its alias

        # Extract the alias mapping for each new curve
        for alias in root:
            if alias.tag in new_curves:
                for curve in alias:
                    original_curve = curve.tag
                    if original_curve in self.keys():
                        alias_mapping.append(alias.tag)
        print(alias_mapping)
        # Check if all new_curves have matching keys in self.keys()
        for new_curve in new_curves:
            if new_curve not in alias_mapping:
                # new_curve doesn't have an alias in the XML
                return False

        # All new_curves have matching keys in self.keys()
        return True
    ...
    def fluid_properties(self, top=0, bottom=100000, mast=67,
        ...
        # Load Rwa if log is available
            ...
            rsp = 10 ** (-1 * self['SP_N'][depth_index] / ksp)
            rmf = (rmft + 6.77) / (form_temp + 6.77) * rmfs

            # Initialize an empty list to store the 'rwa' values for each 'rmf'
            rwa = []

            # Loop through various 'rmf' values
            for rmf_value in rmf:
                rmfe = (rmf_value + 6.77) / (form_temp + 6.77) * rmfs
                if rmf_value > 0.1:
                    rmfe = rmf_value * 0.85
                else:
                    rmfe = (146 * rmf_value - 5) / (337 * rmf_value + 77)
                rwe = rmfe / rsp
                if rwe > 0.12:
                    rwa_value = -1 * (0.58 - 10 ** (0.69 * rwe - 0.24))
                else:
                    rwa_value = (77 * rwe + 5) / (146 - 337 * rwe)
                rwa.append(rwa_value)
        else:
            messagebox.showerror('Error', 'No resistivity or SP log is available.')
            return
    ...
    def formation_fluid_properties(self, parameter='default', formations=None, top=None, bottom=None):
        """
    ...
    def export_excel(self, filepath=None):
        ...
        if filepath is None:
            local_path = os.path.dirname(__file__)
            filepath = os.path.join(local_path, f"output/{self.filename}_las2excel.xlsx")
        if check_file(filepath):
