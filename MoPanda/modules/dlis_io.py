...
def dlis_viewer():
    ...
    def show_channels_data(f, channels):
        ...
        def show_description():
            description_window = tk.Toplevel(channels_window)
            description_window.title('Description')
            description_label = tk.Label(description_window, text=f.describe())
            description_label.pack()

        def show_origin():
            origin_window = tk.Toplevel(channels_window)
            origin_window.title('Origin')
            origin_window.geometry('1200x400')
            origin, *origin_tail = f.origins
            origin_label = tk.Label(origin_window, text=origin.describe())
            origin_label.pack()

        def show_frames():
            frames_window = tk.Toplevel(channels_window)
            frames_window.title('Frames')

            def display_frame_details(frame):
                frame_window = tk.Toplevel(frames_window)
                frame_window.title(f'Frame: {frame.name}')
                frame_window.geometry('1200x400')
                frame_label = tk.Label(frame_window, text=f'Frame Name: {frame.name}\n'
                                                          f'Index Type: {frame.index_type}\n'
                                                          f'Depth Interval: {frame.index_min} - {frame.index_max}\n'
                                                          f'Depth Spacing: {frame.spacing}\n'
                                                          f'Direction: {frame.direction}\n'
                                                          f'Num of Channels: {len(frame.channels)}\n'
                                                          f'Channel Names: {str(frame.channels)}',
                                       wraplength=1100, justify='left')
                frame_label.pack()

                # Update window to calculate the height based on the content
                frame_window.update_idletasks()
                window_height = frame_label.winfo_reqheight()
                frame_window.geometry(f'1200x{window_height}')

            for frame in f.frames:
                frame_button = tk.Button(frames_window, text=f'Frame: {frame.name}',
                                         command=lambda frame=frame: display_frame_details(frame))
                frame_button.pack(padx=10, pady=5)

        def show_tools():
            tools_window = tk.Toplevel(channels_window)
            tools_window.title('Tools')
            tools_window.geometry('1200x400')
            tools = summary_dataframe(f.tools, name='Name', description='Description')
            tools_text = tk.Text(tools_window)
            tools_text.pack(expand=True, fill='both')
            tools_text.insert(tk.END, tools.to_string(index=True))

        def show_wellref():
            wellref_window = tk.Toplevel(channels_window)
            wellref_window.title('Well_References')
            wellref_window.geometry('1200x400')
            wellref_label = tk.Label(wellref_window, text=f.wellref)
            wellref_label.pack()

        description_button = tk.Button(channels_window, text='Description', command=show_description)
        description_button.pack(side='left', padx=10)

        origin_button = tk.Button(channels_window, text='Origin', command=show_origin)
        origin_button.pack(side='left', padx=10)

        frames_button = tk.Button(channels_window, text='Frames', command=show_frames)
        frames_button.pack(side='left', padx=10)

        tools_button = tk.Button(channels_window, text='Tools', command=show_tools)
        tools_button.pack(side='left', padx=10)

        wellref_button = tk.Button(channels_window, text='Wellrefs', command=show_wellref)
        wellref_button.pack(side='left', padx=10)
