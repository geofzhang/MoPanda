...
class WellLogGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("LAS IO GUI")
    ...
    def create_widgets(self):
        ...
        self.text_params = tk.Text(self, width=140, height=50)
        self.text_params.pack(pady=5)
    ...
    def display_params(self):
        if self.log:
            ...
            self.text_params.insert(tk.END, params_info)
            self.update()
        else:
            self.text_params.delete("1.0", tk.END)
            self.text_params.insert(tk.END, "No LAS file loaded.")
            self.update()
